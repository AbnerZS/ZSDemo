...
#define VideoPlayerSrcName(file) [@"Player.bundle" stringByAppendingPathComponent:file]

#define VideoPlayerFrameworkSrcName(file) [@"Frameworks/Player.framework/Player.bundle" stringByAppendingPathComponent:file]

#define VideoPlayerImage(file)      [UIImage imageNamed:VideoPlayerSrcName(file)] ? :[UIImage imageNamed:VideoPlayerFrameworkSrcName(file)]
...
//快进⏩和快退的view
//-(void)creatFF_View{
//    self.FF_View = [[FastForwardView alloc] initWithFrame:CGRectMake(0, 0, 0, 0)];
...
-(void)initVideoPlayer{
    ...
    //创建fastForwardView
   // [self creatFF_View];
    //[[UIApplication sharedApplication].keyWindow addSubview:[VideoLightView sharedLightView]];
    //设置默认值
    self.seekTime = 0.00;
    ...
    self.topView.image = VideoPlayerImage(@"top_shadow");
    ...
    self.bottomView.image = VideoPlayerImage(@"bottom_shadow");
    ...
    [self.playOrPauseBtn setImage:VideoPlayerImage(@"pause") forState:UIControlStateNormal];
    [self.playOrPauseBtn setImage:VideoPlayerImage(@"play") forState:UIControlStateSelected];
    ...
    [self.progressSlider setThumbImage:VideoPlayerImage(@"dot")  forState:UIControlStateNormal];
    ...
    [self.fullScreenBtn setImage:VideoPlayerImage(@"fullscreen") forState:UIControlStateNormal];
    [self.fullScreenBtn setImage:VideoPlayerImage(@"nonfullscreen") forState:UIControlStateSelected];
...
- (void)setPlaceholderImage:(UIImage *)placeholderImage
{
    ...
    if (placeholderImage) {
        ...
        UIImage *image = VideoPlayerImage(@"");
...
- (void)setURLString:(NSString *)URLString{
    ...
    if (self.closeBtnStyle==CloseBtnStylePop) {
        [_closeBtn setImage:VideoPlayerImage(@"play_back.png") forState:UIControlStateNormal];
        [_closeBtn setImage:VideoPlayerImage(@"play_back.png") forState:UIControlStateSelected];
        
    }else{
        [_closeBtn setImage:VideoPlayerImage(@"close") forState:UIControlStateNormal];
        [_closeBtn setImage:VideoPlayerImage(@"close") forState:UIControlStateSelected];
    }
}
            ...
            switch (status)
            {
                ...
                case AVPlayerStatusReadyToPlay:
                {
                    ...
                    if (self.delegate&&[self.delegate respondsToSelector:@selector(videoplayerReadyToPlay:VideoPlayerStatus:)]) {
                        [self.delegate videoplayerReadyToPlay:self VideoPlayerStatus:VideoPlayerStatePlaying];
                    }
                    //here
                ...
                case AVPlayerStatusFailed:
                {
                    ...
                    if (self.delegate&&[self.delegate respondsToSelector:@selector(videoplayerFailedPlay:VideoPlayerStatus:)]) {
                        [self.delegate videoplayerFailedPlay:self VideoPlayerStatus:VideoPlayerStateFailed];
                    }
...
-(void)timeValueChangingWithValue:(float)value{
    if (value > _touchBeginValue) {
        //self.FF_View.sheetStateImageView.image = VideoPlayerImage(@"progress_icon_r");
    }else if(value < _touchBeginValue){
        //self.FF_View.sheetStateImageView.image =VideoPlayerImage(@"progress_icon_l");
    }
...
//        if ([aLightView isKindOfClass:[VideoLightView class]]) {
